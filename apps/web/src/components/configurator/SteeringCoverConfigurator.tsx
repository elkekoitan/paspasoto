...
export default function SteeringCoverConfigurator() {
  ...
  function applyPreset(p: SteeringPreset) {
    const sz = STEERING_SIZES.find((s) => s.slug === p.sizeSlug) ?? size
    const pat = STEERING_PATTERNS.find((pp) => pp.slug === p.patternSlug) ?? pattern
    const mat = STEERING_MATERIALS.find((m) => m.slug === p.materialSlug) ?? material
    setSize(sz)
    setPattern(pat)
    setMaterial(mat)
  }
